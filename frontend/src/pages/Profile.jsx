            ...
            {activeTab === 'settings' && (
              <div className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-col p-4 rounded-2xl border border-slate-100 transition-all">
                    <div className="flex items-center justify-between cursor-pointer group" onClick={() => {
                        import('firebase/auth').then(({ sendPasswordResetEmail }) => {
                          import('../firebase').then(({ auth }) => {
                            if (auth.currentUser && auth.currentUser.email) {
                              sendPasswordResetEmail(auth, auth.currentUser.email)
                                .then(() => alert("Password reset link has been sent to your email. Please check your inbox!"))
                                .catch(err => {
                                  console.error(err);
                                  alert("Failed to send email: " + err.message);
                                });
                            } else {
                              alert("No email associated with this account or not logged in.");
                            }
                          });
                        });
                    }}>
                      <div className="flex items-center gap-4">
                         ...
                         <div>
                            ...
                            <p className="text-xs text-slate-500">Send password reset link to email</p>
                         </div>
                      </div>
                      <ChevronRight className="w-5 h-5 text-slate-300" />
                    </div>
